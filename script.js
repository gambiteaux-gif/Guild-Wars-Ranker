...
let state = loadState();
let selectedWarId = state.wars.length ? state.wars[0].id : "";
...
function loadState() {
  ...
  try {
    ...
    if (!parsed || !Array.isArray(parsed.wars)) {
...
// Older saved data can be missing newer fields. This keeps stale localStorage
// from breaking scoring after the app is updated.
function normalizeState(rawState) {
  rawState.wars = rawState.wars.map((war) => ({
    id: war.id || createId("war"),
    name: war.name || "Untitled War",
    enemy: war.enemy || war.enemyName || "Unknown Enemy",
    date: war.date || "",
    teamType: war.teamType === "Defense" ? "Defense" : "Offense",
    players: Array.isArray(war.players) ? war.players.map(normalizePlayer) : []
  }));
  return rawState;
}
...
function normalizePlayer(entry) {
  return {
    id: entry.id || createId("player"),
    name: entry.name || "Unknown Player",
    role: ["DPS", "Tank", "Healer"].indexOf(entry.role) >= 0 ? entry.role : "DPS",
    team: entry.team === "Defense" ? "Defense" : "Offense",
    isJungler: Boolean(entry.isJungler),
    kills: Number(entry.kills) || 0,
    assists: Number(entry.assists) || 0,
    deaths: Number(entry.deaths) || 0,
    damage: Number(entry.damage) || 0,
    tanking: Number(entry.tanking) || 0,
    healing: Number(entry.healing) || 0,
    siegeDamage: Number(entry.siegeDamage) || 0,
    funCoins: Number(entry.funCoins) || 0
  };
}
...
function scorePlayer(playerData) {
  ...
  if (!rule) {
    return {
      score: 0,
      efficiency: 0,
      usedStats: ["No matching scoring rule"],
      focusStat: 0
    };
  }
...
function averageRank(entries) {
  ...
  const ranks = { S: 6, A: 5, B: 4, C: 3, D: 2, E: 1 };
  const match = Object.keys(ranks).find((rank) => ranks[rank] === closest);
  return match || "-";
}
...
function renderWarSelectors() {
  ...
  selectedWarId = activeWar ? activeWar.id : "";
...
function deleteActiveWar() {
  ...
  selectedWarId = state.wars.length ? state.wars[0].id : "";
  selectedPlayerName = "";
  saveState();
  render();
}
...
inputs.team.value = getActiveWar() ? getActiveWar().teamType : "Offense";
render();
